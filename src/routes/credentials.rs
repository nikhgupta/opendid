...
use crate::{
    constants::AUTHORIZATION_CODE_PROPS,
    routes::authorize::{validate_authorization_code, CodeAuthorizationData},
};
...
async fn post_credential_handler(
    ...
    log::info!("Decrypted message: {:?}", content);
    ...
    for (i, attestation) in attestations.iter().enumerate() {
        ...
        for requirement in requirements {
            ...
            log::info!("{}", &credential_attester_did);
            log::info!("{:?}", content);
            log::info!("{:?}", content.claim.contents);
    ...
    let authorization_code = session
        ...
        .unwrap_or_default();
    log::info!("Authorization code: {:?}", authorization_code);
